...
// lecture (you can even copy this code, no need to type the same code)The Complete JavaScript Course 31

function whereAmI(lat, lng) {
    // Fetch data from geocode.xyz API for reverse geocoding
    fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Problem fetching geocoding data. ${response.status}`);
            }
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error(`Problem fetching country data. ${response.status}`);
            }
            return response.json();
        })
        .then(data => {
